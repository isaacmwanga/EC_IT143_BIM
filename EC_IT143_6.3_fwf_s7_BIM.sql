...
, LEFT(t. ContactName, CHARINDEX(* ', t. ContactName + ' *) - 1) AS first_name , dbo.udf_parse_first_namet.ContactName) AS first_name2
FROM dbo.+_w3_schools_customers AS t)
SELECT s1.*
FROM s1
WHERE s1. first_name < s1. first_name2; -- Expected result is e rows